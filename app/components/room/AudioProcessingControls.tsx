        ...
        <div className={`space-y-6 ${className}`}>
            ...
            <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium text-white flex items-center">
                    ...
                    <span className={`ml-2 px-2 py-1 rounded-full text-xs ${
                        ...
                        {isProcessingActive ? '🎛️ 处理链活跃' : '⏸️ 处理链停止'}
                    </span>
                </h3>
            ...
            <div className="bg-blue-900/20 border border-blue-800 rounded-lg p-3">
                <p className="text-xs text-blue-300">
                    💡 使用 Web Audio API 音频处理链，实时处理而不重建轨道
                </p>
                {!isProcessingActive && (
                    <p className="text-xs text-yellow-300 mt-1">
                        ⚠️ 音频处理链未激活，设置将在初始化后生效
                    </p>
                )}
            </div>
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex-1">
                        ...
                        {settings.echoCancellation && (
                            <div className="mt-1 p-2 bg-blue-900/20 border border-blue-800 rounded text-xs text-blue-300">
                                ⚠️ 此选项需要重新获取音频流才能生效
                            </div>
                        )}
                    </div>
                    <div className="flex items-center space-x-2 ml-4">
                        {isApplying('echoCancellation') && (
                            <div className="flex items-center space-x-1">
                                ...
                                <span className="text-xs text-yellow-400">重新初始化中</span>
                            </div>
                        )}
                        <button
            ...
            <div className="bg-gray-800/50 rounded-lg p-3">
                <h4 className="text-sm font-medium text-gray-300 mb-2">音频处理链状态</h4>
                <div className="grid grid-cols-2 gap-4 text-xs">
                    <div>
                        <div className="flex justify-between">
                            <span className="text-gray-400">处理链:</span>
                ...
                <div className="mt-3 pt-2 border-t border-gray-600">
                    <p className="text-xs text-gray-500">
                        💡 <strong>处理链工作原理：</strong><br/>
                        原始音频 → 增益控制 → 动态压缩 → 噪声滤波 → 输出到服务器
                    </p>
                </div>
            </div>
        </div>
